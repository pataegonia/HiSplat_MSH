...
def train(cfg_dict: DictConfig):
    ...
    checkpoint_path = update_checkpoint_path(cfg.checkpoint_path or cfg.checkpointing.load, cfg.wandb)
    ...
    if cfg.mode == "train" and checkpoint_path is not None and not cfg.checkpointing.resume:
        ...
        if not encoder_state:
            raise RuntimeError(f"No encoder weights found in checkpoint: {checkpoint_path}")
        load_result = encoder.load_state_dict(encoder_state, strict=False)
        unexpected_missing = [k for k in load_result.missing_keys if "gauss_feature_codec" not in k]
        unexpected_missing.extend(load_result.unexpected_keys)
        if unexpected_missing:
            raise RuntimeError(f"Unexpected checkpoint mismatch: {unexpected_missing}")
        if load_result.missing_keys:
            print(cyan(f"Initialized new codec params: {len(load_result.missing_keys)} missing checkpoint keys."))
    ...
    if cfg.mode == "train":
        ...
        fit_ckpt_path = checkpoint_path if cfg.checkpointing.resume else None
        if fit_ckpt_path is not None:
            print(f"resume from {fit_ckpt_path}!!!")
        else:
            print("start training without trainer-state resume!!!")
        trainer.fit(model_wrapper, datamodule=data_module, ckpt_path=fit_ckpt_path)
    elif cfg.mode == "test":
        trainer.test(
            model_wrapper,
