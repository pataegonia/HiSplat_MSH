...
class DepthPredictorMultiViewPyramid(nn.Module):
    ...
    def __init__(
        ...
        use_gauss_feature_compression=False,
        codec_N=192,
        codec_M=320,
        codec_stage_N=None,
        codec_stage_M=None,
        codec_stage_depth=None,
        codec_train_mode="full",
        codec_input="proj",
        **kwargs,
    ):
        ...
        for stage_id in range(len(channel_list)):
            stage_codec_N = codec_stage_N[stage_id] if codec_stage_N is not None else codec_N
            stage_codec_M = codec_stage_M[stage_id] if codec_stage_M is not None else codec_M
            stage_codec_depth = codec_stage_depth[stage_id] if codec_stage_depth is not None else 4
            depth_predictor_list.append(
                DepthPredictorRefine(
                    ...
                    num_views,
                    stage_id,
                    use_gauss_feature_compression=use_gauss_feature_compression,
                    codec_N=stage_codec_N,
                    codec_M=stage_codec_M,
                    codec_depth=stage_codec_depth,
                    codec_train_mode=codec_train_mode,
    ...
    def forward(
        ...
        for i in range(last_stage + 1):
            ...
            if codec_payloads is not None:
                stage_payload = codec_payloads.get(f"stage{i}")

            depths, densities, raw_gaussians, coarse_disps, pdf_max, codec_info = depth_predictor(
                trans_feature_i,
                cnn_feature[i],
                intrinsics,
                extrinsics,
            ...
            result_dict[f"stage{i}"]["estimated_bits"] = None if codec_info is None else codec_info.get("estimated_bits")
            result_dict[f"stage{i}"]["codec_recon_loss"] = (
                None if codec_info is None else codec_info.get("recon_loss")
            )
            result_dict[f"stage{i}"]["codec_payload"] = None if codec_info is None else codec_info.get("payload")
...
class DepthPredictorRefine(nn.Module):
    ...
    def __init__(
        ...
        if codec_train_mode == "entropy":
            codec_train_mode = "full"
        if codec_train_mode not in {"transform", "quantize", "quantize_hp", "full"}:
            raise ValueError(f"Unknown codec_train_mode: {codec_train_mode}")
        if codec_input not in {"proj", "stack"}:
            raise ValueError(f"Unknown codec_input: {codec_input}")
        ...
        self.codec_train_mode = codec_train_mode
        self.codec_input = codec_input
        self.stage_stack_channels = channel_list[channel_stage_id] + 2 + (0 if stage_id == 0 else 3)
        self.codec_channels = channel_list[channel_stage_id] if codec_input == "proj" else self.stage_stack_channels
        ...
        if use_gauss_feature_compression:
            from ..compression import StageMeanScaleHyperprior

            self.gauss_feature_codec = StageMeanScaleHyperprior(
                in_channels=self.codec_channels,
                N=codec_N,
                M=codec_M,
                analysis_depth=codec_depth,
            )
        else:
            self.gauss_feature_codec = None

    def _pack_stage_stack(self, proj_feat_in_fullres, extra_img, coarse_disps, pdf_max, pre_stage_residual):
        if self.stage_id == 0:
            return torch.cat([proj_feat_in_fullres, coarse_disps, pdf_max], dim=1)
        return torch.cat([proj_feat_in_fullres, coarse_disps, pdf_max, pre_stage_residual], dim=1)

    def _unpack_stage_stack(self, feat_stack, c_proj):
        if self.stage_id == 0:
    ...
    def _apply_codec(self, feat_stack, codec_mode, codec_payload, extra_info=None):
        ...
        if codec_mode in {"forward", "bypass"}:
            codec_out = self.gauss_feature_codec(feat_stack, mode=self.codec_train_mode)
            x_hat = codec_out["x_hat"]
            self._check_codec_output(x_hat, codec_mode)
            recon_loss = self._codec_recon_loss(x_hat, feat_stack)
            return_feat = feat_stack if codec_mode == "bypass" else x_hat
            return return_feat, {
                "estimated_bits": codec_out.get("estimated_bits"),
                "recon_loss": recon_loss,
                "codec_train_mode": self.codec_train_mode,
                "codec_input": self.codec_input,
                "render_bypassed": codec_mode == "bypass",
            }
        ...
        if codec_mode == "compress":
            ...
            x_hat = decoded["x_hat"]
            self._check_codec_output(x_hat, codec_mode)
            return x_hat, {"payload": payload}

        if codec_mode == "decompress":
            if codec_payload is None:
                raise ValueError(f"Missing codec payload for stage{self.stage_id}.")
            decoded = self.gauss_feature_codec.decompress(**codec_payload)
            x_hat = decoded["x_hat"]
            self._check_codec_output(x_hat, codec_mode)
            return x_hat, {"payload": codec_payload}

        if codec_mode in ("none", None):
            return feat_stack, None

        raise ValueError(f"Unknown codec_mode: {codec_mode}")

    @staticmethod
    def _codec_recon_loss(x_hat, feat_stack):
        target = feat_stack.detach()
        energy = target.pow(2).mean().clamp_min(1e-6)
        return F.mse_loss(x_hat, target) / energy

    def _check_codec_output(self, x_hat, codec_mode):
        if not torch.isfinite(x_hat).all():
            finite = x_hat[torch.isfinite(x_hat)]
            if finite.numel() == 0:
                stats = "no finite values"
            else:
                stats = f"finite_min={finite.min().item():.6g}, finite_max={finite.max().item():.6g}"
            raise FloatingPointError(
                f"Stage {self.stage_id} codec produced non-finite x_hat "
                f"in {codec_mode}/{self.codec_train_mode} mode ({stats})."
            )
        max_abs = x_hat.detach().abs().amax()
        if max_abs > 1.0e4:
            raise FloatingPointError(
                f"Stage {self.stage_id} codec x_hat magnitude exploded "
                f"in {codec_mode}/{self.codec_train_mode} mode: {max_abs.item():.6g}."
            )
    ...
    def forward(
        ...
        if self.codec_input == "proj":
            proj_feat_in_fullres, codec_info = self._apply_codec(
        ...
        return depths, densities, raw_gaussians, coarse_disps, pdf_max, codec_info
